...
def check_availability(sku: str, start: str, end: str, location: str) -> Dict[str, Any]:
    with SessionLocal() as s:
        ...
        ), {"sku": sku, "loc": location}).mappings().first()
        if not row: return {"available": False, "reason":"SKU/Location not found"}
...
def get_rate(sku: str, duration_days: int, customer_tier: str) -> Dict[str, Any]:
    with SessionLocal() as s:
        row = s.execute(text("SELECT daily, weekly, monthly, damage_waiver_pct, delivery_fee_base FROM rates WHERE sku=:sku"),
...
def policy_guardrails(quote: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if quote.get("weekend_delivery", False):
        ...
        subtotal += 75.00
    return {"violations": [], "applied_adjustments": adjustments, "subtotal": round(subtotal,2)}
